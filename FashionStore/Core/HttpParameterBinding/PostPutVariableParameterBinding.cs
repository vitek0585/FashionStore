...
namespace FashionStore.Core.HttpParameterBinding
{
    public class PostPutVariableParameterBinding : System.Web.Http.Controllers.HttpParameterBinding
    {
        private const string MultipleBodyParameters = "MultipleBodyParameters";

        public PostPutVariableParameterBinding(HttpParameterDescriptor descriptor)
            : base(descriptor)
        {
        }
        ...
        private object StringToType(string stringValue)
        {
            ...
            {
                value = string.IsNullOrEmpty(stringValue) ? new string[] { } : stringValue.Split(',');
            }
            else
                value = stringValue;
